...
class Kriteria2AdminController extends Controller
{
    public function index()
    {
        $data = DetailKriteriaModel::with('kriteria')->where('id_kriteria', 2)->get();
        return view('kriteria.admin.kriteria2.index', compact('data'));
    }

    public function create()
    {
        return view('kriteria.admin.kriteria2.create');
    }

    public function edit(string $id)
    {
        $kriteria = DetailKriteriaModel::with('penetapan', 'pelaksanaan', 'evaluasi', 'pengendalian', 'peningkatan', 'komentar')->findOrFail($id);
        return view('kriteria.admin.kriteria2.edit', [
            'kriteria' => $kriteria
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('index.admin.kriteria2')->with('success', 'Data berhasil diperbarui sebagai ' . ($action === 'draft' ? 'draf' : 'submit') . '.');
    }
    ...
    public function show($id)
    {
        ...
        return view('kriteria.admin.kriteria2.view', compact('kriteria'));
    }

    public function print($id)
    {
        $item = DetailKriteriaModel::with('kriteria')->findOrFail($id);
        $pdf = Pdf::loadView('kriteria.admin.kriteria2.printPdf', compact('item'));

        return $pdf->download('Laporan_Kriteria2_' . $item->id_detail_kriteria . '.pdf');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('index.admin.kriteria2')->with('success', 'Data berhasil dihapus.');
    }
}