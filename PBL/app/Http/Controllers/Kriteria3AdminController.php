...
class Kriteria3AdminController extends Controller
{
    public function index()
    {
        $data = DetailKriteriaModel::with('kriteria')->where('id_kriteria', 3)->get();
        return view('kriteria.admin.kriteria3.index', compact('data'));
    }

    public function create()
    {
        return view('kriteria.admin.kriteria3.create');
    }

    public function edit(string $id)
    {
        $kriteria = DetailKriteriaModel::with('penetapan', 'pelaksanaan', 'evaluasi', 'pengendalian', 'peningkatan', 'komentar')->findOrFail($id);
        return view('kriteria.admin.kriteria3.edit', [
            'kriteria' => $kriteria
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
       ...
       if ($isUpdated) {
        ...
        return redirect()->route('index.admin.kriteria3')->with('success', 'Data berhasil diperbarui.');
    }
    ...
    public function storePenetapan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            ...
            $fileName = time() . '_' . Str::random(30) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('penetapan_kriteria3', $fileName, 'public');
        }
    ...
    public function storePelaksanaan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(30) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pelaksanaan_kriteria3', $fileName, 'public');
        }
    ...
    public function storeEvaluasi(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(30) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('evaluasi_kriteria3', $fileName, 'public');
        }
    ...
    public function storePengendalian(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(30) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pengendalian_kriteria3', $fileName, 'public');
        }
    ...
    public function submitKriteria(Request $request)
    {
        ...
        return redirect()->route('index.admin.kriteria3')->with('success', 'Data berhasil disimpan sebagai ' . ($action === 'draft' ? 'draf' : 'submit') . '.');
    }
    ...
    public function show($id)
    {
        ...
        return view('kriteria.admin.kriteria3.view', compact('kriteria'));
    }

    public function print($id)
    {
        $item = DetailKriteriaModel::with('kriteria')->findOrFail($id);
        $pdf = Pdf::loadView('kriteria.admin.kriteria3.printPdf', compact('item'));

        return $pdf->download('Laporan_Kriteria3_' . $item->id_detail_kriteria . '.pdf');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('index.admin.kriteria3')->with('success', 'Data berhasil dihapus.');
    }
}