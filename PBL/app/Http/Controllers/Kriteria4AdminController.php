...
class Kriteria4AdminController extends Controller
{
    public function index()
    {
        $data = DetailKriteriaModel::with('kriteria')->where('id_kriteria', 4)->get();
        return view('kriteria.admin.kriteria4.index', compact('data'));
    }

    public function create()
    {
        return view('kriteria.admin.kriteria4.create');
    }

    public function edit(string $id)
    {
        $kriteria = DetailKriteriaModel::with('penetapan', 'pelaksanaan', 'evaluasi', 'pengendalian', 'peningkatan', 'komentar')->findOrFail($id);
        return view('kriteria.admin.kriteria4.edit', [
            'kriteria' => $kriteria
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('index.admin.kriteria4')->with('success', 'Data berhasil diperbarui sebagai ' . ($action === 'draft' ? 'draf' : 'submit') . '.');
    }
    ...
    public function storePenetapan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            ...
            $filePath = $file->storeAs('penetapan_kriteria4', $fileName, 'public');
        }
    ...
    public function storePelaksanaan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(5) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pelaksanaan_kriteria4', $fileName, 'public');
        }
    ...
    public function storeEvaluasi(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(5) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('evaluasi_kriteria4', $fileName, 'public');
        }
    ...
    public function storePengendalian(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(5) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pengendalian_kriteria4', $fileName, 'public');
        }
    ...
    public function storePeningkatan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(5) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('peningkatan_kriteria4', $fileName, 'public');
        }
    ...
    public function submitKriteria(Request $request)
    {
        ...
        return redirect()->route('index.admin.kriteria4')->with('success', 'Data berhasil disimpan sebagai ' . ($action === 'draft' ? 'draf' : 'submit') . '.');
    }
    ...
    public function show($id)
    {
        ...
        return view('kriteria.admin.kriteria4.view', compact('kriteria'));
    }

    public function print($id)
    {
        $item = DetailKriteriaModel::with('kriteria')->findOrFail($id);
        $pdf = Pdf::loadView('kriteria.admin.kriteria4.printPdf', compact('item'));

        return $pdf->download('Laporan_Kriteria4_' . $item->id_detail_kriteria . '.pdf');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('index.admin.kriteria4')->with('success', 'Data berhasil dihapus.');
    }
}