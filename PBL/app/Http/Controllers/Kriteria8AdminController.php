...
class Kriteria8AdminController extends Controller
{
    public function index()
    {
        $data = DetailKriteriaModel::with('kriteria')->where('id_kriteria', 8)->get();
        return view('kriteria.admin.kriteria8.index', compact('data'));
    }

    public function create()
    {
        return view('kriteria.admin.kriteria8.create');
    }

    public function edit(string $id)
    {
        $kriteria = DetailKriteriaModel::with('penetapan', 'pelaksanaan', 'evaluasi', 'pengendalian', 'peningkatan', 'komentar')->findOrFail($id);
        return view('kriteria.admin.kriteria8.edit', [
            'kriteria' => $kriteria
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
       ...
       if ($isUpdated) {
        ...
        return redirect()->route('index.admin.kriteria8')->with('success', 'Data berhasil diperbarui.');
    }
    ...
    public function storePenetapan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            ...
            $fileName = time() . '_' . Str::random(80) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('penetapan_kriteria8', $fileName, 'public');
        }
    ...
    public function storePelaksanaan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(80) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pelaksanaan_kriteria8', $fileName, 'public');
        }
    ...
    public function storeEvaluasi(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(80) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('evaluasi_kriteria8', $fileName, 'public');
        }
    ...
    public function storePengendalian(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(80) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('pengendalian_kriteria8', $fileName, 'public');
        }
    ...
    public function storePeningkatan(Request $request)
    {
        ...
        if ($request->hasFile('pendukung')) {
            $file = $request->file('pendukung');
            $fileName = time() . '_' . Str::random(80) . '.' . $file->getClientOriginalExtension();
            $filePath = $file->storeAs('peningkatan_kriteria8', $fileName, 'public');
        }
    ...
    public function submitKriteria(Request $request)
    {
        ...
        return redirect()->route('index.admin.kriteria8')->with('success', 'Data berhasil disimpan sebagai ' . ($action === 'draft' ? 'draf' : 'submit') . '.');
    }
    ...
    public function show($id)
    {
        ...
        return view('kriteria.admin.kriteria8.view', compact('kriteria'));
    }

    public function print($id)
    {
        $item = DetailKriteriaModel::with('kriteria')->findOrFail($id);
        $pdf = Pdf::loadView('kriteria.admin.kriteria8.printPdf', compact('item'));

        return $pdf->download('Laporan_Kriteria8_' . $item->id_detail_kriteria . '.pdf');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('index.admin.kriteria8')->with('success', 'Data berhasil dihapus.');
    }
}