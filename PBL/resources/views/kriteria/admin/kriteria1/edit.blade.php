...
@section('content')
    ...
    <form id="kriteria1-edit" method="POST" action="{{ route('kriteria1.update', $kriteria->id_detail_kriteria) }}" enctype="multipart/form-data" {{ $disabled }}>
        @csrf
        @method('PUT')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const form = document.querySelector('#kriteria1-edit');
                const submitUrl = '{{ route('kriteria1.update', $kriteria->id_detail_kriteria) }}';
                ...
                form.addEventListener('submit', function (e) {
                                ...
                                fetch(window.location.href, { method: 'GET' })
                                    ...
                                    .then(html => {
                                        if (html.includes('Perubahan berhasil disimpan')) {
                                            ...
                                            setTimeout(() => {
                                                window.location.href = '{{ route('index.admin.kriteria1') }}';
                                            }, 2000);
                                        }
                                    });
                            });
                    ...
                    .then(data => {
                        if (data.status) {
                            ...
                            setTimeout(() => {
                                window.location.href = data.redirect || '{{ route('index.admin.kriteria1') }}';
                            }, 2000);
                        }
                    })
                    .catch(error => {
