...
@section('content')
    ...
    <form id="kriteria3-edit" method="POST" action="{{ route('kriteria3.update', $kriteria->id_detail_kriteria) }}" enctype="multipart/form-data" {{ $disabled }}>
        @csrf
        @method('PUT')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const form = document.querySelector('#kriteria3-edit');
                const submitUrl = '{{ route('kriteria3.update', $kriteria->id_detail_kriteria) }}';
                ...
                form.addEventListener('submit', function (e) {
                                ...
                                fetch(window.location.href, { method: 'GET' })
                                    ...
                                    .then(html => {
                                        if (html.includes('Data berhasil diperbarui')) {
                                            ...
                                            setTimeout(() => {
                                                window.location.href = '{{ route('index.admin.kriteria3') }}';
                                            }, 2000);
                                        }
                                    });
                            });
                    ...
                    .then(data => {
                        if (data.status) {
                            ...
                            setTimeout(() => {
                                window.location.href = data.redirect || '{{ route('index.admin.kriteria3') }}';
                            }, 2000);
                        }
                    })
                    .catch(error => {
