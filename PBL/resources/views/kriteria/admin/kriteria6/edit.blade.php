...
@section('content')
    <!-- Header -->
    <div class="header">
        <h3>Beranda / Kriteria 6 / Edit</h3>
        <h2>Edit Kriteria 6</h2>
    </div>
    ...
    <form id="kriteria6-edit" method="POST" action="{{ route('kriteria6.update', $kriteria->id_detail_kriteria) }}" enctype="multipart/form-data" {{ $disabled }}>
        @csrf
        @method('PUT')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const form = document.querySelector('#kriteria6-edit');
                const submitUrl = '{{ route('kriteria6.update', $kriteria->id_detail_kriteria) }}';
                ...
                form.addEventListener('submit', function (e) {
                                ...
                                fetch(window.location.href, { method: 'GET' })
                                    ...
                                    .then(html => {
                                        if (html.includes('Data berhasil diperbarui')) {
                                            ...
                                            setTimeout(() => {
                                                window.location.href = '{{ route('index.admin.kriteria6') }}';
                                            }, 2000);
                                        }
                                    });
                            });
                    ...
                    .then(data => {
                        if (data.status) {
                            ...
                            setTimeout(() => {
                                window.location.href = data.redirect || '{{ route('index.admin.kriteria6') }}';
                            }, 2000);
                        }
                    })
                    .catch(error => {
