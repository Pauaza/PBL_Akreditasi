...
@section('content')
    <!-- Header -->
    <div class="header">
        <h3>Beranda / Kriteria 7 / Edit</h3>
        <h2>Edit Kriteria 7</h2>
    </div>
    ...
    <form id="kriteria7-edit" method="POST" action="{{ route('kriteria7.update', $kriteria->id_detail_kriteria) }}" enctype="multipart/form-data" {{ $disabled }}>
        @csrf
        @method('PUT')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const form = document.querySelector('#kriteria7-edit');
                const submitUrl = '{{ route('kriteria7.update', $kriteria->id_detail_kriteria) }}';
                ...
                form.addEventListener('submit', function (e) {
                                ...
                                fetch(window.location.href, { method: 'GET' })
                                    ...
                                    .then(html => {
                                        if (html.includes('Data berhasil diperbarui')) {
                                            ...
                                            setTimeout(() => {
                                                window.location.href = '{{ route('index.admin.kriteria7') }}';
                                            }, 2000);
                                        }
                                    });
                            });
                    ...
                    .then(data => {
                        if (data.status) {
                            ...
                            setTimeout(() => {
                                window.location.href = data.redirect || '{{ route('index.admin.kriteria7') }}';
                            }, 2000);
                        }
                    })
                    .catch(error => {
